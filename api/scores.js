module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!event) return res.status(200).json({ players: [], status: 'no_data', debug: 'no events' });
    ...
    // Parse any score value ESPN might send — number, string, "E", "-3", "+2"
    function parseToPar(val) {
      ...
      // Handle "EVEN"
      if (s.toUpperCase() === 'EVEN') return 0;
      const n = parseInt(s, 10);
      if (isNaN(n)) return 0;
      // If raw strokes (e.g. 68, 70), convert to to-par
      if (n > 30) return n - COURSE_PAR;
      return n;
    }
    ...
    const leaderboard = competitors.map(function(comp) {
      ...
      // ESPN sends score/today as strings with +/- prefix or "E"

module.exports = async function handler(req, res) {
  ...
  try {
    // Try the leaderboard endpoint (different from scoreboard)
    const url = `https://site.api.espn.com/apis/site/v2/sports/golf/leaderboard?league=pga&tournamentId=${TOURNAMENT_ID}`;
    ...
    if (!response.ok) {
      ...
      const url2 = `https://site.api.espn.com/apis/site/v2/sports/golf/pga/scoreboard?tournamentId=${TOURNAMENT_ID}`;
      const r2 = await fetch(url2, {
        headers: { 'User-Agent': 'Mozilla/5.0', 'Accept': 'application/json' }
      });
      if (!r2.ok) throw new Error(`Both endpoints failed: ${response.status} / ${r2.status}`);
      const d2 = await r2.json();
      return processData(d2, COURSE_PAR, res);
    }

    const data = await response.json();
    return processData(data, COURSE_PAR, res);
...
function processData(data, COURSE_PAR, res) {
  const event = data && data.events && data.events[0];
  if (!event) return res.status(200).json({ players: [], status: 'no_event_data' });
  ...
  const leaderboard = competitors.map(function(comp) {
    ...
    return {
      ...
      todayScore: (!isCut && !isWD && !isDQ) ? (todayToPar !== null ? todayToPar : 0) : null,
      thru: thru,
